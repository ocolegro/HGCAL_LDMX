...
def main(): 
	...
	os.system("cp ~/geant4_workdir/bin/Linux-g++/PFCalEE ~/geant4_workdir/tmp/Linux-g++/PFCalEE/libPFCalEE.so g4env4lpc.sh userlib/lib/libPFCalEEuserlib.so generators/electronGen.py %s/." % subdir)
	os.chdir(subdir);
	os.system("tar -cvzf inputs.tar.gz PFCalEE g4env4lpc.sh libPFCalEE.so libPFCalEEuserlib.so electronGen.py")
	...
	for i in range(njobs):
		...
		f1.write("python electronGen.py -n %s -f \"events\" -e 4 \n" % opt.nevtsperjob)
		f1.write("ls -lrt \n");
